...
class ShellTokenizer():
    ...
    def tokenize_corpus(self, corpus):
        l = len(corpus)
        for i, command in tqdm(enumerate(corpus), desc=f"[*] Tokenizing commands", total=l):
            self.tokenized_corpus.append(self.tokenize_command(command, i=i))
        if self.verbose:
            logging.info(f"[!] Finished tokenizing {l} commands | Final bashlex error ratio: {self.BASHLEX_ERR*100/l:.2f}%")
...
class ShellEncoder():
    def __init__(self, token_counter: Counter, top_tokens: int = 100, verbose: bool = False):
    ...
    def tfidf(self, tokenized_corpus):
        # Inverse-Data-Frequency (IDF)
        idf = defaultdict(float)
        for token in self.top_token_list:
            appearance_in_corpus = 0
            for tokenized_cmd in tokenized_corpus:
                if token in tokenized_cmd:
                    appearance_in_corpus += 1
            # 1 + is needed to not divide by 0 when there is no token in corpus
            idf[token] = np.log((1 + len(tokenized_corpus)) /(1 + appearance_in_corpus))

        # TF-IDF
        tfidf = np.zeros((len(tokenized_corpus), self.top_tokens))
        logging.info(f"[!] Starting TF-IDF encoding!")
        for i,tokenized_cmd in tqdm(enumerate(tokenized_corpus), desc="[*] TF-IDF encoding", total=len(tokenized_corpus)):
            omc = Counter(tokenized_cmd)
            tf = defaultdict(float)            
            for token in set(tokenized_cmd):
                # Term Frequency (TF) == Bag of Words (BoW)
                tf[token] = omc[token]/len(tokenized_cmd)
    ...
    def labels(self, tokenized_corpus: List, pad_width: int = None):          
        self.top_token_list = [x[0] for x in self.most_common]
        local_corpus = tokenized_corpus.copy()

        le = LabelEncoder().fit(self.top_token_list+["OTHER"])
        for i, tokenized_cmd in enumerate(tokenized_corpus):
            local_corpus[i] = le.transform([x if x in self.top_token_list else "OTHER" for x in tokenized_cmd])
        ...
        for i,x in tqdm(enumerate(local_corpus), desc="[*] Label encoding", total=len(tokenized_corpus)):
            z = np.zeros(max_pad)
    ...
    def onehot(self, tokenized_corpus):
        output = np.zeros(shape=(len(tokenized_corpus), self.top_tokens), dtype=int)
        logging.info(f"[!] Starting One-Hot encoding!")
        for i, tokenized_cmd in tqdm(enumerate(tokenized_corpus), desc="[*] One-Hot encoding", total=len(tokenized_corpus)):
